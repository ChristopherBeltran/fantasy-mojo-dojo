...
export default async function MatchupsPage() {
  ...
  const posters = week
    ? await prisma.matchupPoster.findMany({
        where: { leagueId: league.id, week },
      })
    : [];
  const posterByPair = new Map(
    posters.map((p) => [`${p.managerAId}:${p.managerBId}`, p]),
  );
  ...
  return (
    <PageShell activeTab="matchups" leagueName={league.name}>
      ...
      <p className="text-sm text-muted mb-5">
        {week ? `Week ${week}` : "No games synced yet"}
      </p>
      ...
      {pairs.length === 0 ? (
        <div className="bg-card border border-border rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-muted">
            Run a sync once the season has games to see this week&apos;s
            pairings.
          </p>
        </div>
      ) : (
        // Two-up on desktop — posters are tall portraits, so full-width cards
        // made each one fill more than a screen. Single column on phones.
              ...
              >
                {poster && (
                  <div className="relative">
                    ...
                    <img
                      src={poster.imageUrl}
                      alt={`${homeName} vs ${awayName} matchup poster`}
                      className="w-full"
                    />
                ...
                <div className="px-5 py-4 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 tabular font-bold shrink-0">
                    <span
                      className={
                        homeWinning ? "text-brandTeal" : "text-slate-300"
                      }
                    >
                      {m.points.toFixed(1)}
                    </span>
